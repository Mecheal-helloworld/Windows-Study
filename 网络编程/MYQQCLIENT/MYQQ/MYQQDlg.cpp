...
void CMYQQDlg::OnBnClickedButton1()
{
	...
	ifget = m_sConnectSocket.Create(); //创建客户端套接字对象的底层套接字，使用默认的参数s
	if (ifget == true) {
		// 调用套接字类的成员函数，连接到服务器
		if (true != m_sConnectSocket.Connect(m_SerName, m_SerPort)) {
			AfxMessageBox(TEXT("连接失败！"), MB_OK | MB_ICONSTOP);
			OnClose();
		}
	}
	else {
